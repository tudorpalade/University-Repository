...
public class Rook extends Piece {
	...
	private final int[] candidateLegalMoves = {-8,-1,1,8};
	
	public Rook(Alliance color,Tile tile) {
		super(color, tile);
	}
	...
	private boolean isFirstColumnExclusion( final int tileNo , final int candidateDestination) {
		if (BoardUtils.isInColumn(1, tileNo) && ((candidateDestination == -1 )))
			return true;
		return false;
	}
	
	private boolean isEightColumnExclusion( final int tileNo , final int candidateDestination) {
		if (BoardUtils.isInColumn(8, tileNo) && ((candidateDestination == 1 )))
			return true;
		return false;
	}
